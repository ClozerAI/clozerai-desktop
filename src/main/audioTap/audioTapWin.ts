...
const checkWindowsVersion = (): void => {
  ...
  if (major < 6) {
    throw new Error(
      `ClozerAI requires Windows Vista (NT 6.0) or later. Current system: NT ${release}. You can use the web version of ClozerAI instead.`,
    );
  }
};
...
const getWindowsArchitecture = (): string => {
  ...
  switch (arch) {
    ...
    default:
      throw new Error(
        `Unsupported Windows architecture: ${arch}. ClozerAI Desktop supports x64, x86, and ARM64 architectures.`,
      );
  }
};
